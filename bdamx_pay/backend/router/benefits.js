...
var benefits = require('../model/benefits');
	...
	try{
		...
		async.parallel({
			bannerTotalCount:function(cb) {      
			  benefits.find( cnt ).countDocuments().exec(cb);
			},
			bannerData:function(cb) {
			  benefits.find( cnt ).limit(limit).skip(skip).sort( srt ).exec(cb);
	   	},
...
router.get('/getnews', (req, res) => {
	try {
			common.redisConfig('benefits', '', (resData) => {
				var resp=[];
...
router.post('/getnewspage', (req, res) => {
	var data = req.body;
	try{
		benefits.find({title:data.title}).sort({_id:-1}).exec(function(err,news){
...
router.post('/faqupload',(req, res) => {
	try{
		...
		benefits.create(values, function (err, resData) {
...
router.post('/updateimage', (req, res) => {
	try{
		db.updateOne({},{ $set:{"image":req.body.image}}).exec(function(err, resUpdate){
				common.SetredisConfig('benefits', '', (resData) => {
					if(err){
...
router.post('/updateimage_nw', (req, res) => {
  benefits.updateOne({ "_id": req.body._id },{$set:{
		...
		try
		{
			common.SetredisConfig('benefits', '', (resData) => {
				if(err){
					...
					benefits.find({ "_id": req.body._id }).exec(function(err,resData){
						if (err) return res.status(500).send(err);
						apiresponse.data = [];
						apiresponse.data = resData;
						apiresponse.message = "Benefits  Updated Successfully";
						res.json(apiresponse);
					});
				}
			});				
...
router.post('/faqstatus', (req,res) => {
	try{
		benefits.updateOne({ "_id": req.body._id },{ "$set": { "status": req.body.status,
...
router.post('/deletefaq', (req, res) => {
	...
	benefits.findOneAndRemove({"_id": req.body._id}).exec(function(err,resData){
		try{
			common.SetredisConfig('benefits', '', (resData) => {
				if(err){
					res.json({ "status": false});
					...
					apiresponse.data    = [];
					apiresponse.data    = resData;
					apiresponse.message = "Benefits has been  deleted successfully";
					res.json(apiresponse);
				}
			});				
		}

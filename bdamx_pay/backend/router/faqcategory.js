...
var faqcategory = require('../model/faqcategory');
	...
	try{
		...
		async.parallel({
			bannerTotalCount:function(cb) {      
			  faqcategory.find( cnt ).countDocuments().exec(cb);
			},
			bannerData:function(cb) {
			  faqcategory.find( cnt ).limit(limit).skip(skip).sort( srt ).exec(cb);
	   	},
...
router.get('/getfaqcategory', (req, res) => {
  try{
  	faqcategory.find({},function(err,categoryList){
  		res.json({status:true,data:categoryList})
  	})
  }
  catch(e){
    console.log('getfaqcategory',e);
  }
});


router.post('/addFaqCategory', (req, res) => {
  try{
  	faqcategory.create(req.body,function(err,categoryList){
  		res.json({status:true,data:categoryList})
  	})
  }
  catch(e){
...
router.get('/getnews', (req, res) => {
	try {
			common.redisConfig('faqcategory', '', (resData) => {
				var resp=[];
...
router.get('/getnewspage', (req, res) => {
	try{
		faqcategory.find({"status":0},{'title':1,"description":1,"modifieddate":1,"content":1}).sort({_id:-1}).exec(function(err,news){
...
router.post('/faqupload',(req, res) => {
	try{
		var values = req.body;
		faqcategory.create(values, function (err, resData) {
...
router.post('/updateimage', (req, res) => {
	try{
		db.updateOne({},{ $set:{"image":req.body.image}}).exec(function(err, resUpdate){
				common.SetredisConfig('faqcategory', '', (resData) => {
					if(err){
...
router.post('/updateimage_nw', (req, res) => {
  faqcategory.updateOne({ "_id": req.body._id },{$set:{
		...
		try
		{
			common.SetredisConfig('faqcategory', '', (resData) => {
				if(err){
					...
					faqcategory.find({ "_id": req.body._id }).exec(function(err,resData){
						if (err) return res.status(500).send(err);
						apiresponse.data = [];
						apiresponse.data = resData;
						apiresponse.message = "FAQ Category  Updated Successfully";
						res.json(apiresponse);
					});
				}
			});				
...
router.post('/faqstatus', (req,res) => {
	try{
		faqcategory.updateOne({ "_id": req.body._id },{ "$set": { "status": req.body.status,
...
router.post('/deletefaq', (req, res) => {
	...
	faqcategory.findOneAndRemove({"_id": req.body._id}).exec(function(err,resData){
		try{
			common.SetredisConfig('faqcategory', '', (resData) => {
				if(err){
					res.json({ "status": false});
					...
					apiresponse.data    = [];
					apiresponse.data    = resData;
					apiresponse.message = "FAQ Category has been  deleted successfully";
					res.json(apiresponse);
				}
			});				
		}

...
var smartcase_table = mongoose.model('smartcase');
	...
	try{
		...
		async.parallel({
			bannerTotalCount:function(cb) {      
			  smartcase_table.find( cnt ).countDocuments().exec(cb);
			},
			bannerData:function(cb) {
			  smartcase_table.find( cnt ).limit(limit).skip(skip).sort( srt ).exec(cb);
	   	},
...
router.get('/getnewspage', (req, res) => {
	try{
		smartcase_table.find({"status":0},{'title':1,"description":1,"modifieddate":1,"content":1}).sort({_id:-1}).exec(function(err,news){
...
router.post('/faqupload',(req, res) => {
	try{
		var values = req.body;
		smartcase_table.create(values, function (err, resData) {
...
router.post('/updateimage_nw', (req, res) => {
  smartcase_table.updateOne({ "_id": req.body._id },{$set:{
		...
		try
		{
			common.SetredisConfig('learnmore', '', (resData) => {
				if(err){
					...
					smartcase_table.find({ "_id": req.body._id }).exec(function(err,resData){
						if (err) return res.status(500).send(err);
						apiresponse.data = [];
...
router.post('/faqstatus', (req,res) => {
	try{
		smartcase_table.updateOne({ "_id": req.body._id },{ "$set": { "status": req.body.status,
...
router.post('/deletefaq', (req, res) => {
	...
	smartcase_table.findOneAndRemove({"_id": req.body._id}).exec(function(err,resData){
		try{
			common.SetredisConfig('learnmore', '', (resData) => {
				if(err){
					res.json({ "status": false});

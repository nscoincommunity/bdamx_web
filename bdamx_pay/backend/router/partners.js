	...
	try{
		...
		async.parallel({
			bannerTotalCount:function(cb) {      
			  partners.find( cnt ).countDocuments().exec(cb);
			},
			bannerData:function(cb) {
			  partners.find( cnt ).limit(limit).skip(skip).sort( srt ).exec(cb);
	   	},
...
router.get('/getnews', (req, res) => {
	try {
			common.redisConfig('partners', '', (resData) => {
				var resp=[];
...
router.get('/getnewspage', (req, res) => {
	try{
		partners.find({"status":0},{'title':1,"description":1,"modifieddate":1,"content":1, "image":1}).sort({_id:1}).exec(function(err,news){
...
router.post('/faqupload',(req, res) => {
	...
	try{
		var values = req.body;
		partners.create(values, function (err, resData) {
...
router.post('/updateimage', (req, res) => {
	try{
		db.updateOne({},{ $set:{"image":req.body.image}}).exec(function(err, resUpdate){
				common.SetredisConfig('partners', '', (resData) => {
					if(err){
...
router.post('/updateimage_nw', (req, res) => {
  partners.updateOne({ "_id": req.body._id },{$set:{
		...
		try
		{
			common.SetredisConfig('partners', '', (resData) => {
				if(err){
					...
					partners.find({ "_id": req.body._id }).exec(function(err,resData){
						if (err) return res.status(500).send(err);
						apiresponse.data = [];
						apiresponse.data = resData;
						apiresponse.message = "Partners  Updated Successfully";
						res.json(apiresponse);
					});
				}
			});				
...
router.post('/faqstatus', (req,res) => {
	try{
		partners.updateOne({ "_id": req.body._id },{ "$set": { "status": req.body.status,
...
router.post('/deletefaq', (req, res) => {
	...
	partners.findOneAndRemove({"_id": req.body._id}).exec(function(err,resData){
		try{
			common.SetredisConfig('partners', '', (resData) => {
				if(err){
					res.json({ "status": false});
					...
					apiresponse.data    = [];
					apiresponse.data    = resData;
					apiresponse.message = "Partners has been  deleted successfully";
					res.json(apiresponse);
				}
			});				
		}

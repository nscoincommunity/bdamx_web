...
var markets = require('../model/markets');
	...
	try{
		...
		async.parallel({
			bannerTotalCount:function(cb) {      
			  markets.find( cnt ).countDocuments().exec(cb);
			},
			bannerData:function(cb) {
			  markets.find( cnt ).limit(limit).skip(skip).sort( srt ).exec(cb);
	   	},
...
router.get('/getnews', (req, res) => {
	try {
			common.redisConfig('markets', '', (resData) => {
				var resp=[];
...
router.post('/getnewspage', (req, res) => {
	var data = req.body
	try{
		markets.find({title:data.title}).sort({_id:1}).exec(function(err,news){
...
router.post('/faqupload',(req, res) => {
	try{
		var values = req.body;
		markets.create(values, function (err, resData) {
...
router.post('/updateimage', (req, res) => {
	try{
		db.updateOne({},{ $set:{"image":req.body.image}}).exec(function(err, resUpdate){
				common.SetredisConfig('markets', '', (resData) => {
					if(err){
...
router.post('/updateimage_nw', (req, res) => {
  markets.updateOne({ "_id": req.body._id },{$set:{
		...
		try
		{
			common.SetredisConfig('markets', '', (resData) => {
				if(err){
					...
					markets.find({ "_id": req.body._id }).exec(function(err,resData){
						if (err) return res.status(500).send(err);
						apiresponse.data = [];
...
router.post('/faqstatus', (req,res) => {
	try{
		markets.updateOne({ "_id": req.body._id },{ "$set": { "status": req.body.status,
...
router.post('/deletefaq', (req, res) => {
	...
	markets.findOneAndRemove({"_id": req.body._id}).exec(function(err,resData){
		try{
			common.SetredisConfig('markets', '', (resData) => {
				if(err){
					res.json({ "status": false});

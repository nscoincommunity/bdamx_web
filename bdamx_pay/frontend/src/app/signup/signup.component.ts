...
export class SignupComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    var rlink = this.route.snapshot.paramMap.get('id')
    if( rlink!=null ){
      console.log("rlink",rlink)
      this.apiService.loader(true);
      this.emailverify(rlink)
    }
  }


  FieldsChange(values:any){
    this.reginput.terms=values.currentTarget.checked
  ...
  public executeImportantAction(method, callback): void {
    ...
    this.recaptchaV3Service.execute(method)
      .subscribe(token =>{ 
        console.log(this.recaptchaV3Service,'resultrecaptchaaaa');
  ...
  emailverify(data){
    ...
    this.executeImportantAction('verify_email',(response)=>{
      var SenData = {
        user_id : data,
        captcha_value:this.captcha_value
      } 
      this.apiService.SenData("exchange/verify_email",SenData).subscribe((resData:any) => {
        if(resData.status){
  ...
  regSubmit(form: NgForm){
    ...
    this.executeImportantAction('register',(response)=>{
    this.reginput.captcha_value = this.captcha_value
      this.apiService.SenData("exchange/enroll",this.reginput).subscribe((resData:any) => {
        if(resData.status){

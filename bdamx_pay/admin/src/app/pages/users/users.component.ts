...
export class UsersComponent implements OnInit {
  ...
  onSubmit() {
	...
	openeditmodel(content,bannerrow) {
	    ...
	    this.get_walletdetails(this.editbanners._id)
	...
	openview(bannerrow, row) {
      this.editbanners = row;
      this.modalRef = this.modalService.open(bannerrow,{size:'lg'});
  }

  change_tfastatus(row){
  	this.CommonService.requestData('custom/change_tfa_status',{_id:row._id,tfaenable:row.tfaenabled}).subscribe(resData=>{
  		if(resData.status){
  			this.toastr.success('TFA disabled successfully');
  			this.currencyloadfn();
  		}else{
  			this.toastr.error('Unable to disable TFA');
  		}
  	})
  }

userWalletlength : any =0
	get_walletdetails(id){
		console.log(id)
		let sendData = {
			getter : id
		}
			this.CommonService.requestData('BdAMx4fdex/get_balance',sendData).subscribe(resData => {
				if(resData.status){

					this.userWalletdetails = resData.result
					this.userWalletlength = this.userWalletdetails.length 
					console.log(this.userWalletdetails)
	...
	download_walletcsv(){
		console.log(this.userWalletdetails.length,this.userWalletdetails,this.editbanners)

		var download_data=[]
		for(var i=0;i<this.userWalletdetails.length;i++){
		var datas={
		Currency:this.userWalletdetails[i].currencyname,
		Address:this.userWalletdetails[i].currencyaddress,
		Balance:this.userWalletdetails[i].c_type==1 ? this.userWalletdetails[i].currencybalance.toFixed(8) : this.userWalletdetails[i].currencybalance.toFixed(2),
		...
		var Userexport = download_data;
		var options={
		headers: ["Currency", "Address", "Balance"]
		}
		
		new ngxCsv(Userexport, this.editbanners.email + new Date().getTime(),options);
  ...
  opennewsmodel(model_name){
	...
	change_status(detail,sts){
		...
		this.CommonService.requestData('BdAMx4fdex/changests',req).subscribe(resData=>{
			...
			this.socket.emit('deactivateuser', resData)
		})
	}

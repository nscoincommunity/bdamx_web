...
export class MainbannerComponent implements OnInit {
	...
	@ViewChild('currencyimage') myInputVariable1: ElementRef;
  ...
  reset1() {
	...
	currencyloadfn(){
		...
		this.CommonService.requestData('currencydetails/getallcurrency',lstinput)
		.subscribe(resData => {
  ...
  setPage_banner(pageInfo){
	...
	filenames = "";
	fileChangeEvents(fileInput: any, type) {
		this.filesToUploads=[];
  ...
  onSubmit(type) {
    ...
    .subscribe(files => {
        this.CommonService.requestData('currencydetails/updateimage',{"image":files.result.secure_url}).subscribe(res=>{
        	if(type === 'currencyImage'){
        		this.editbanners.image = files.result.secure_url;
        	} else {
        		this.editbanners.coinImage = files.result.secure_url;
        	}
	...
	change_currency_status(value,status){
		...
		this.CommonService.requestData('currencydetails/currencystatus',req)
		.subscribe(resData => {		
			this.currencyloadfn();
	...
	// change_feetype(data){
	// 	this.editbanners.feetype = +data;
	// }
	update_currency(){
		console.log(this.editbanners, 'bannersss');
		this.CommonService.requestData('currencydetails/update_currency',this.editbanners)
		.subscribe(resData => {
	...
	download_csv(){
		...
		this.CommonService.requestData('currencydetails/getallcurrency',lstinput).subscribe(resData => {
			if(resData){
				var download_data=[]

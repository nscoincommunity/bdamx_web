    ...
    if(this.ban.title.trim()=='' && this.ban.description.trim()==''){
      ...
      console.log(req,'requestobject');
        ...
        if (this.modalRef) {
            ...
            this.toastr.error('Vision & Mission are not updated!', 'Error');
          }
        });
    }
  }
...
//edit pop up display
  ...
  saveEditEmailTemplate(content){
    ...
    .subscribe(resData => {
      ...
      this.newsloadfn();
      this.toastr.success('Vision & Mission updated Successfully!', 'Success');
      this.modalRef.close();
    });
  }
...
changebannerStatus(editbannervalue) {
      ...
      this.CommonService.requestData('news/faqstatus', this.currencyObj).subscribe(resData => {
        if(resData.data)
        {
          this.toastr.success('Vision & Mission status changed Successfully!', 'Success');
         if (this.modalRef) {
                  this.modalRef.close()
              }
              this.newsloadfn();
...
deletenews(deletevalue){
    ...
    this.CommonService.requestData('news/deletefaq',deletevalue)
      .subscribe(resData => {
        ...
        this.toastr.error('Vision & Mission is not deleted!', 'Error');

...
export class IpblockComponent implements OnInit {
 ...
 faqloadfn(){
    ...
    this.CommonService.requestData('ipblock/getiplist',lstinput)
    .subscribe(resData => {
...
//end of the function  
  ...
  ipSubmit(form: NgForm){
    this.ipsubmitted = true
     let SendData = {
      blocker : this.ipinput.ipadd
    }

     if(this.ipinput.ipadd.trim()==''){
      this.toastr.error('Enter Valid Ip Address', 'Error');
      this.ipsubmitted = false
    }
    else{

      this.CommonService.requestData('ipblock/ip_add',SendData)
      .subscribe(resData => {
        if(resData.status){
          if (this.modalRef) {
          this.modalRef.close()
...
changebannerStatus(editbannervalue) {
      ...
      this.currencyObj = {"_id":editbannervalue._id,"ip":editbannervalue.ip,"status": this.banner_Status};
      this.CommonService.requestData('custom/ipstatus', this.currencyObj).subscribe(resData => {
        if(resData.data)
        {
          this.toastr.success('IP status changed Successfully!', 'Success');
         if (this.modalRef) {
                  this.modalRef.close()
              }
              this.faqloadfn();

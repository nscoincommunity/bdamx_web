...
@ViewChild(DatatableComponent) marketstable:DatatableComponent;
  ...
  onSubmit_nw(type) {
    ...
    .subscribe(files => {
        this.CommonService.requestData('markets/updateimage_nw',{"image":files.result.secure_url,"_id":this.editbanners._id}).subscribe(res=>{
          this.loader=false
 ...
 newsloadfn(){
    ...
    this.CommonService.requestData('markets/getfaqlist',lstinput)
    .subscribe(resData => {
   ...
   Add_news(value){
    if(this.ban.title.trim()=='' && this.ban.description.trim()==''){
      ...
      console.log(req,'requestobject');
        ...
        this.CommonService.requestData('markets/faqupload', req)
        .subscribe(resData => {
          if (resData) {
                     this.toastr.success('Market Details added successfully!', 'Success');
        if (this.modalRef) {
                    this.modalRef.close()
                }
                this.newsloadfn();
            ...
            this.toastr.error('Market Details are not updated!', 'Error');
          }
        });
    }
  }
...
//edit pop up display
  ...
  saveEditEmailTemplate(content){
    ...
    this.CommonService.requestData('markets/faqlist',req)
    .subscribe(resData => {
      this.bannerblocktemp = this.bannerblocklist;
      this.bannerblockrows = this.bannerblocklist;
      this.bannerblockrows = [...this.bannerblockrows];
      this.newsloadfn();
      this.toastr.success('Market Details updated Successfully!', 'Success');
      this.modalRef.close();
    });
  }
...
changebannerStatus(editbannervalue) {
      ...
      this.CommonService.requestData('markets/faqstatus', this.currencyObj).subscribe(resData => {
        if(resData.data)
        {
          this.toastr.success('Market Details status changed Successfully!', 'Success');
         if (this.modalRef) {
                  this.modalRef.close()
              }
              this.newsloadfn();
...
deletenews(deletevalue){
    ...
    this.CommonService.requestData('markets/deletefaq',deletevalue)
      .subscribe(resData => {
        if(resData.data){
                  this.toastr.success('Market Details is deleted!', 'Success');
        ...
        this.toastr.error('Market Details is not deleted!', 'Error');

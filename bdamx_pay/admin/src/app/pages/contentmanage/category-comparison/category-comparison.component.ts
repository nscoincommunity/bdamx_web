...
@Component({
  selector: 'ngx-category-comparison',
  templateUrl: './category-comparison.component.html',
  styleUrls: ['./category-comparison.component.scss']
})
export class CategoryComparisonComponent implements OnInit {
 ...
 newsloadfn(){
    ...
    this.CommonService.requestData('compare/getcmprcategoryList',lstinput)
    .subscribe(resData => {
...
//change the row limit
 ...
 //save the image after upload
   Add_news(value){
    ...
    // else{
      ...
      console.log(req,'requestobject');
        ...
        this.CommonService.requestData('compare/addCategory', req)
        .subscribe(resData => {
          if (resData) {
  ...
  saveEditEmailTemplate(content){
    ...
    this.CommonService.requestData('compare/editCategory',req)
    .subscribe(resData => {
      this.bannerblocktemp = this.bannerblocklist;
      this.bannerblockrows = this.bannerblocklist;
...
changebannerStatus(editbannervalue) {
      ...
      this.CommonService.requestData('compare/categorystatus', this.currencyObj).subscribe(resData => {
        if(resData.data)
        {
          this.toastr.success('Category status changed Successfully!', 'Success');
         if (this.modalRef) {
                  this.modalRef.close()
              }
              this.newsloadfn();
...
deletenews(deletevalue){
    ...
    this.CommonService.requestData('compare/deletecategory',deletevalue)
      .subscribe(resData => {
        if(resData.data){
        ...
        this.toastr.error('Category is not deleted!', 'Error');

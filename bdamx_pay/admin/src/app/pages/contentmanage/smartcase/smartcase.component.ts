...
@Component({
  selector: 'ngx-smartcase',
  templateUrl: './smartcase.component.html',
  styleUrls: ['./smartcase.component.scss']
})
export class SmartcaseComponent implements OnInit {

  
  
@ViewChild(DatatableComponent) featurestable:DatatableComponent;
  ...
  headingFromsubmitted:boolean=false
  headingcontent:any={}
  ...
  onSubmit_nw(type) {
    ...
    .subscribe(files => {
        this.CommonService.requestData('smartcase/updateimage_nw',{"image":files.result.secure_url,"_id":this.editbanners._id}).subscribe(res=>{
          this.loader=false
 ...
 newsloadfn(){
    ...
    this.CommonService.requestData('smartcase/getfaqlist',lstinput)
    .subscribe(resData => {
   ...
   Add_news(value){
    if(this.ban.title.trim()=='' && this.ban.description.trim()==''){
      ...
      var req={
        ...
        this.CommonService.requestData('smartcase/faqupload', req)
        .subscribe(resData => {
          if (resData) {
                     this.toastr.success('Learn more  added successfully!', 'Success');
        if (this.modalRef) {
                    this.modalRef.close()
                }
                this.newsloadfn();
            ...
            this.toastr.error('Learn more are not updated!', 'Error');
          }
        });
    }
  }
...
//edit pop up display
  ...
  saveEditEmailTemplate(content){
    ...
    this.CommonService.requestData('smartcase/faqlist',req)
    .subscribe(resData => {
      this.bannerblocktemp = this.bannerblocklist;
      this.bannerblockrows = this.bannerblocklist;
...
changebannerStatus(editbannervalue) {
      ...
      this.CommonService.requestData('smartcase/faqstatus', this.currencyObj).subscribe(resData => {
        if(resData.data)
        {
          this.toastr.success('Feature status changed Successfully!', 'Success');
         if (this.modalRef) {
                  this.modalRef.close()
              }
              this.newsloadfn();
...
deletenews(deletevalue){
    ...
    this.CommonService.requestData('smartcase/deletefaq',deletevalue)
      .subscribe(resData => {
        if(resData.data){
                  this.toastr.success('Feature is deleted!', 'Success');
        ...
        this.toastr.error('Feature is not deleted!', 'Error');

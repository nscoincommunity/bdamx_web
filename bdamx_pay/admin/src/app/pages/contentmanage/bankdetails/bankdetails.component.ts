...
export class BankdetailsComponent implements OnInit {
 ...
 faqloadfn(){
    ...
    this.CommonService.requestData('bankDetails/getbanklist',lstinput)
    .subscribe(resData => {
...
//end of the function  
  ...
  currencybank : any =[]
  getcurrencybank(data){
    console.log(data)
    // let senData = {
    //   coin : data
    // }
    // this.CommonService.requestData('adminbankinfo/getcurrencybank',senData)
    // .subscribe(resData => {
    //   if(resData.status){
 ...
 //save the image after upload
   Add_bankinfo(form: NgForm){
    ...
    // else{
        ...
        this.CommonService.requestData('bankDetails/addbank',this.ban)
        .subscribe(resData => {
          if (resData.status) {
                     this.toastr.success('Bank Detail is added successfully!', 'Success');
                     console.log('modalrefffff', this.modalRef)
        if (this.modalRef) {
        	console.log('insidemodalrefffff')
                    this.modalRef.close()
                }
                this.faqloadfn();
                form.resetForm(); 
...
//edit pop up display
  ...
  saveEditEmailTemplate(content,bannerrow){
    ...
    // }
    // else{    
    this.CommonService.requestData('bankDetails/updatebanklist',this.editbanners)
    .subscribe(resData => {
    	if(resData.status){
    		this.bannerblocktemp = this.bannerblocklist;
	     	this.bannerblockrows = this.bannerblocklist;
...
changebannerStatus(editbannervalue) {
      ...
      this.CommonService.requestData('bankDetails/detailstatus', this.currencyObj).subscribe(resData => {
        if(resData.data)
        {

...
export class FaqcategoryComponent implements OnInit {
  ...
  @ViewChild(DatatableComponent) faqcategorytable:DatatableComponent;
  ...
  onSubmit_nw(type) {
    ...
    .subscribe(files => {
        this.CommonService.requestData('faqcategory/updateimage_nw',{"image":files.result.secure_url,"_id":this.editbanners._id}).subscribe(res=>{
          this.loader=false
 ...
 newsloadfn(){
    ...
    this.CommonService.requestData('faqcategory/getfaqlist',lstinput)
    .subscribe(resData => {
   ...
   Add_news(value){
    if(this.ban.title.trim()=='' && this.ban.description.trim()==''){
      ...
      console.log(req,'requestobject');
        ...
        this.CommonService.requestData('faqcategory/faqupload', req)
        .subscribe(resData => {
          if (resData) {
                     this.toastr.success('FAQ category  added successfully!', 'Success');
        if (this.modalRef) {
                    this.modalRef.close()
                }
                this.newsloadfn();
            ...
            this.toastr.error('FAQ category are not updated!', 'Error');
          }
        });
    }
  }
...
//edit pop up display
  ...
  saveEditEmailTemplate(content){
    ...
    this.CommonService.requestData('faqcategory/faqlist',req)
    .subscribe(resData => {
      this.bannerblocktemp = this.bannerblocklist;
      this.bannerblockrows = this.bannerblocklist;
      this.bannerblockrows = [...this.bannerblockrows];
      this.newsloadfn();
      this.toastr.success('FAQ category updated Successfully!', 'Success');
      this.modalRef.close();
    });
  }
...
changebannerStatus(editbannervalue) {
      ...
      this.CommonService.requestData('faqcategory/faqstatus', this.currencyObj).subscribe(resData => {
        if(resData.data)
        {
          this.toastr.success('FAQ category status changed Successfully!', 'Success');
         if (this.modalRef) {
                  this.modalRef.close()
              }
              this.newsloadfn();
...
deletenews(deletevalue){
    ...
    this.CommonService.requestData('faqcategory/deletefaq',deletevalue)
      .subscribe(resData => {
        if(resData.data){
                  this.toastr.success('FAQ category is deleted!', 'Success');
        ...
        this.toastr.error('FAQ category is not deleted!', 'Error');

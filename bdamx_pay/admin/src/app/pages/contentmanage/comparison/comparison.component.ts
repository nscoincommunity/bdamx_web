...
export class ComparisonComponent implements OnInit {
 ...
 @ViewChild(DatatableComponent) paidintable:DatatableComponent;
  ...
  ngOnInit() {
    ...
    this.CommonService.getData('compare/getcmprcategory').subscribe(resData=>{
      this.categoryList = resData.data;
    });
    this.CommonService.getData('compare/getcmprtype').subscribe(resData=>{
 ...
 newsloadfn(){
    ...
    this.CommonService.requestData('compare/getcomparison',lstinput)
    .subscribe(resData => {
...
//change the row limit
  ...
  resetbanneradd(){
    this.editbanners = {"_id":"", "categoryName":"", "type":"","value":""};;
    this.newsloadfn();
  }
 ...
 //save the image after upload
   Add_news(value){
    ...
    // else{
      var req={
        "categoryName":this.editbanners.categoryName,
        "type":this.editbanners.type,
        "value":this.editbanners.value
      }
      console.log(req,'requestobject');

        this.CommonService.requestData('compare/addComparison', req)
        .subscribe(resData => {
          if (resData) {
                     this.toastr.success('Comparison  added successfully!', 'Success');
        if (this.modalRef) {
                    this.modalRef.close()
                }
                this.newsloadfn();
            ...
            this.toastr.error('Comparison not updated!', 'Error');
          }
        });
    // }
  }
...
//edit pop up display
   editbanners = {"_id":"", "categoryName":"", "type":"","value":""};
  ...
  saveEditEmailTemplate(content){
    ...
    this.CommonService.requestData('compare/editcomparison',req)
    .subscribe(resData => {
      this.bannerblocktemp = this.bannerblocklist;
      this.bannerblockrows = this.bannerblocklist;
...
changebannerStatus(editbannervalue) {
      ...
      this.CommonService.requestData('compare/comparisonstatus', this.currencyObj).subscribe(resData => {
        if(resData.data)
        {
          this.toastr.success('Comparison status changed Successfully!', 'Success');
         if (this.modalRef) {
                  this.modalRef.close()
              }
              this.newsloadfn();
...
deletenews(deletevalue){
    ...
    this.CommonService.requestData('compare/deletecomparison',deletevalue)
      .subscribe(resData => {
        if(resData.data){
                  this.toastr.success('Comparison is deleted!', 'Success');
        ...
        this.toastr.error('Comparison is not deleted!', 'Error');

...
import 'ckeditor';
@Component({
  selector: 'ngx-adminbankinfo',
  templateUrl: './adminbankinfo.component.html',
  styleUrls: ['./adminbankinfo.component.scss']
})
export class AdminbankinfoComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.CommonService.getData('currencydetails/getCurrencydet').subscribe(resData=>{
      ...
      //this.getbankinfo(this.currencyList[0].currencysymbol);
    });
 ...
 faqloadfn(){
    ...
    this.CommonService.requestData('adminbankinfo/getadminbanklist',lstinput)
    .subscribe(resData => {
...
//end of the function  
  ...
  currencybank : any =[]
  getcurrencybank(data, type){
    console.log(data)
    let senData = {
      country : data
    }
    this.CommonService.requestData('bankDetails/getbanks',senData)
    .subscribe(resData => {
      if(resData.status){
        this.currencybank = resData.data
      }else{
  ...
  getbankdetails(data, type){
    let senData = {
      country : type == 'add' ? this.ban.country : this.editbanners.country,
      bank:data
    }
    this.CommonService.requestData('bankDetails/getbranch',senData)
    .subscribe(resData => {
      if(resData.status){
        this.branches = resData.data
      }else{
        this.toastr.error(resData.Msg, 'Error');
      }
    });
  ...
  getbranchdetails(data,type){
    let senData = {
      country : type == 'add' ? this.ban.country : this.editbanners.country,
      bank: type == 'add' ? this.ban.bank : this.editbanners.bank,
      branch:data
    }
    this.CommonService.requestData('bankDetails/getdetail',senData)
    .subscribe(resData => {
      if(resData.status){
        this.ban.ifsccode = resData.data.ifsccode;
      }else{
        this.toastr.error(resData.Msg, 'Error');
      }
    });
   ...
   Add_bankinfo(form: NgForm){
    ...
    if(this.ban.currencysymbol.trim()=='' && this.ban.bank.trim()==''){
      this.toastr.error('Enter any content', 'Error');
        ...
        this.CommonService.requestData('adminbankinfo/bankupload',this.ban)
        .subscribe(resData => {
          if (resData) {
        ...
        if (this.modalRef) {
            ...
            this.toastr.error('Bank Detail are not updated!', 'Error');
          }
        });
    }
  }
...
//edit pop up display
   ...
   openeditmodel(content,bannerrow) {
      ...
      this.getcurrencybank(this.editbanners.country);
      this.getbankdetails(this.editbanners.bank);
      this.getbranchdetails(this.editbanners.branch);
      this.modalRef = this.modalService.open(content,{size:'lg'});
        this.modalRef.result.then((result) => {
           this.closeResult = `Closed with: ${result}`;
        }, (reason) => {
  ...
  saveEditEmailTemplate(content,bannerrow){
    ...
    // }
    // else{    
    this.CommonService.requestData('adminbankinfo/banklist',this.editbanners)
    .subscribe(resData => {
      // if(resData.status){
        this.bannerblocktemp = this.bannerblocklist;
        this.bannerblockrows = this.bannerblocklist;
...
changebannerStatus(editbannervalue) {
      ...
      this.CommonService.requestData('adminbankinfo/detailstatus', this.currencyObj).subscribe(resData => {
        if(resData.data)
        {

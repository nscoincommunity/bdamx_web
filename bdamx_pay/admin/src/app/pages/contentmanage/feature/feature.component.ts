...
@ViewChild(DatatableComponent) featurestable:DatatableComponent;
  ...
  onSubmit_nw(type) {
    ...
    .subscribe(files => {
        this.CommonService.requestData('feature/updateimage_nw',{"image":files.result.secure_url,"_id":this.editbanners._id}).subscribe(res=>{
          this.loader=false
 ...
 newsloadfn(){
    ...
    this.CommonService.requestData('feature/getfaqlist',lstinput)
    .subscribe(resData => {
   ...
   Add_news(value){
    if(this.ban.title.trim()=='' && this.ban.description.trim()==''){
      ...
      console.log(req,'requestobject');
        ...
        this.CommonService.requestData('feature/faqupload', req)
        .subscribe(resData => {
          if (resData) {
                     this.toastr.success('Platform Feature added successfully!', 'Success');
        if (this.modalRef) {
                    this.modalRef.close()
                }
                this.newsloadfn();
            ...
            this.toastr.error('Platform Feature are not updated!', 'Error');
          }
        });
    }
  }
...
//edit pop up display
  ...
  saveEditEmailTemplate(content){
    ...
    this.CommonService.requestData('feature/faqlist',req)
    .subscribe(resData => {
      this.bannerblocktemp = this.bannerblocklist;
      this.bannerblockrows = this.bannerblocklist;
      this.bannerblockrows = [...this.bannerblockrows];
      this.newsloadfn();
      this.toastr.success('Platform Feature updated Successfully!', 'Success');
      this.modalRef.close();
    });
  }
...
changebannerStatus(editbannervalue) {
      ...
      this.CommonService.requestData('feature/faqstatus', this.currencyObj).subscribe(resData => {
        if(resData.data)
        {
          this.toastr.success('Platform Feature status changed Successfully!', 'Success');
         if (this.modalRef) {
                  this.modalRef.close()
              }
              this.newsloadfn();
...
deletenews(deletevalue){
    ...
    this.CommonService.requestData('feature/deletefaq',deletevalue)
      .subscribe(resData => {
        if(resData.data){
                  this.toastr.success('Platform Feature is deleted!', 'Success');
        ...
        this.toastr.error('Platform Feature is not deleted!', 'Error');

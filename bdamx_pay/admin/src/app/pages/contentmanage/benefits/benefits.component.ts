...
export class BenefitsComponent implements OnInit {
  ...
  @ViewChild(DatatableComponent) benefitstable:DatatableComponent;
  ...
  onSubmit_nw(type) {
    ...
    .subscribe(files => {
        this.CommonService.requestData('benefits/updateimage_nw',{"image":files.result.secure_url,"_id":this.editbanners._id}).subscribe(res=>{
          this.loader=false
 ...
 newsloadfn(){
    ...
    this.CommonService.requestData('benefits/getfaqlist',lstinput)
    .subscribe(resData => {
   ...
   Add_news(value){
    if(this.ban.title.trim()=='' && this.ban.description.trim()==''){
      ...
      console.log(req,'requestobject');
        ...
        this.CommonService.requestData('benefits/faqupload', req)
        .subscribe(resData => {
          if (resData) {
                     this.toastr.success('Benefits added successfully!', 'Success');
        if (this.modalRef) {
                    this.modalRef.close()
                }
                this.newsloadfn();
            ...
            this.toastr.error('Benefits are not updated!', 'Error');
          }
        });
    }
  }
...
//edit pop up display
  ...
  saveEditEmailTemplate(content){
    ...
    this.CommonService.requestData('benefits/faqlist',req)
    .subscribe(resData => {
      this.bannerblocktemp = this.bannerblocklist;
      this.bannerblockrows = this.bannerblocklist;
      this.bannerblockrows = [...this.bannerblockrows];
      this.newsloadfn();
      this.toastr.success('Benefits updated Successfully!', 'Success');
      this.modalRef.close();
    });
  }
...
changebannerStatus(editbannervalue) {
      ...
      this.CommonService.requestData('benefits/faqstatus', this.currencyObj).subscribe(resData => {
        if(resData.data)
        {
          this.toastr.success('Benefits status changed Successfully!', 'Success');
         if (this.modalRef) {
                  this.modalRef.close()
              }
              this.newsloadfn();
...
deletenews(deletevalue){
    ...
    this.CommonService.requestData('benefits/deletefaq',deletevalue)
      .subscribe(resData => {
        if(resData.data){
                  this.toastr.success('Benefits is deleted!', 'Success');
        ...
        this.toastr.error('Benefits is not deleted!', 'Error');

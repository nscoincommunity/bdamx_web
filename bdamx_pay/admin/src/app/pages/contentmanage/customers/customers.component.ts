...
export class CustomersComponent implements OnInit {
  ...
  @ViewChild(DatatableComponent) customerstable:DatatableComponent;
  ...
  onSubmit_nw(type) {
    ...
    .subscribe(files => {
        this.CommonService.requestData('customers/updateimage_nw',{"image":files.result.secure_url,"_id":this.editbanners._id}).subscribe(res=>{
          this.loader=false
 ...
 newsloadfn(){
    ...
    this.CommonService.requestData('customers/getfaqlist',lstinput)
    .subscribe(resData => {
   ...
   Add_news(value){
    ...
    // else{
      ...
      console.log(req,'requestobject');
        ...
        this.CommonService.requestData('customers/faqupload', req)
        .subscribe(resData => {
          if (resData) {
        ...
        if (this.modalRef) {
            ...
            this.toastr.error('Paid in are not updated!', 'Error');
          }
        });
    // }
  }
...
//edit pop up display
  ...
  saveEditEmailTemplate(content){
    ...
    this.CommonService.requestData('customers/faqlist',req)
    .subscribe(resData => {
      this.bannerblocktemp = this.bannerblocklist;
      this.bannerblockrows = this.bannerblocklist;
...
changebannerStatus(editbannervalue) {
      ...
      this.CommonService.requestData('customers/faqstatus', this.currencyObj).subscribe(resData => {
        if(resData.data)
        {
          this.toastr.success('Paid in status changed Successfully!', 'Success');
         if (this.modalRef) {
                  this.modalRef.close()
              }
              this.newsloadfn();
...
deletenews(deletevalue){
    ...
    this.CommonService.requestData('customers/deletefaq',deletevalue)
      .subscribe(resData => {
        if(resData.data){
                  this.toastr.success('Press is deleted!', 'Success');
        ...
        this.toastr.error('Press is not deleted!', 'Error');

...
@Component({
  ...
  template: `    
      ...
      <small class="form-text sub-title">Enter your email adress and we’ll send your password</small>
    ...
    this.CommonService.requestData('admin/forgetPwdAdmin',this.user)
      .subscribe(resData => {
        if(resData.status == true) {
           ...
           this.user = '';
          this.toastr.success('Reset password is send to your mail!', 'Success!');
          ...
          this.submitted = false;
          this.toastr.error('Please correct your email', 'Oops!');          
        }
    });
  }
  ngOnInit(){
  this.CommonService.getData('siteSettings/getContent').subscribe(resData => {  
   
      this.sitelogo = resData.data[0].frontendlogo;
    });

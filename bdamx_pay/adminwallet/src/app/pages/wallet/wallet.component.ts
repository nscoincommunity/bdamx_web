...
export class WalletComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    btcDetails(){
      this.CommonService.getData('admin/getadminaddress').subscribe(resData => {
        this.btc_address = resData.btc_address;
        if(resData.btc_balance)
        {
           this.originalBTC = resData.btc_balance;
        }
        this.img_qr = 'https://chart.googleapis.com/chart?cht=qr&chs=150x150&chl=' + this.btc_address + '&choe=UTF-8&chld=L';
  ...
  currencyloadfn(){
  	if(this.tableType == 'withdraw'){
	    var lstinput = {"page":this.page_banner,"sorting":this.defsort_banner,"search": this.defsearch_banner,"currency":"BTC"};
  ...
  loadbannerlist(page,sort,search){
  	if(this.tableType == 'withdraw'){  	
    this.loading_banner = true;
    var lstinput = {"page":page,"sorting":sort,"search":search, "currency":"BTC"};
    this.CommonService.requestData('BdAMx4fdex/getadminwithdraw',lstinput)
    .subscribe(resData => {
      this.page_banner.totalElements = resData.count;
      this.bannerblocklist = resData.Message;
  ...
  copyText(val: string) {
	...
	Excuteorder(form: NgForm){
		var req={
			"toAddress":this.withdrw.address,
			"useraddress":this.btc_address,
			"withamount":this.withdrw.amount,
			"type":'admin'
		}
		this.CommonService.requestData('BdAMx4fdex/btcwithdraw',req).subscribe(res=>{
			if(res.status == true){

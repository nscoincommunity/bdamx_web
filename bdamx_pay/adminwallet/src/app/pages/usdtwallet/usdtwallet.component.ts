...
@Component({
  selector: 'ngx-usdtwallet',
  templateUrl: './usdtwallet.component.html',
  styleUrls: ['./usdtwallet.component.scss']
})
export class UsdtwalletComponent implements OnInit {
  ...
  usdtDetails(){
    this.CommonService.getData('admin/getadminaddress').subscribe(resData => {
      this.usdt_address = resData.usdt_address;
      if(resData.usdt_balance)
      {
         this.originalUSDT = resData.usdt_balance;
      }
      this.img_qr = 'https://chart.googleapis.com/chart?cht=qr&chs=150x150&chl=' + this.usdt_address + '&choe=UTF-8&chld=L';
  ...
  currencyloadfn(){
  	if(this.tableType == 'withdraw'){
	    var lstinput = {"page":this.page_banner,"sorting":this.defsort_banner,"search": this.defsearch_banner,"currency":"USDT"};
  ...
  loadbannerlist(page,sort,search){
  	if(this.tableType == 'withdraw'){  	
    this.loading_banner = true;
    var lstinput = {"page":page,"sorting":sort,"search":search,"currency":"USDT"};
    this.CommonService.requestData('BdAMx4fdex/getadminwithdraw',lstinput)
    .subscribe(resData => {
      this.page_banner.totalElements = resData.count;
      this.bannerblocklist = resData.Message;

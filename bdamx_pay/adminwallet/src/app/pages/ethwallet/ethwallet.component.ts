...
@Component({
  selector: 'ngx-ethwallet',
  templateUrl: './ethwallet.component.html',
  styleUrls: ['./ethwallet.component.scss']
})
export class EthwalletComponent implements OnInit {
  ...
  ethDetails(){
    this.CommonService.getData('admin/getadminaddress').subscribe(resData => {
      this.eth_address = resData.eth_address;
      if(resData.eth_balance)
      {
         this.originalETH = resData.eth_balance;
      }
      this.img_qr = 'https://chart.googleapis.com/chart?cht=qr&chs=150x150&chl=' + this.eth_address + '&choe=UTF-8&chld=L';
  ...
  currencyloadfn(){
  	if(this.tableType == 'withdraw'){
	    var lstinput = {"page":this.page_banner,"sorting":this.defsort_banner,"search": this.defsearch_banner,"currency":"ETH"};
  ...
  loadbannerlist(page,sort,search){
  	if(this.tableType == 'withdraw'){  	
    this.loading_banner = true;
    var lstinput = {"page":page,"sorting":sort,"search":search, "currency":"ETH"};
    this.CommonService.requestData('BdAMx4fdex/getadminwithdraw',lstinput)
    .subscribe(resData => {
      this.page_banner.totalElements = resData.count;
      this.bannerblocklist = resData.Message;
  ...
  copyText(val: string) {
	...
	Excuteorder(form: NgForm){
		var req={
			...
			"useraddress":this.eth_address,
			"withamount":this.withdrw.amount
		}
		this.CommonService.requestData('custom/ethwithdraw',req).subscribe(res=>{
			if(res.status == true){

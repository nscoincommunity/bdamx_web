...
export class ProfitComponent implements OnInit {
  ...
  defsort: any = {dir: "desc", prop: "date"};
  defsearch = "";
  ...
  defsort_banner: any = {dir: "desc", prop: "date"};
  defsearch_banner = "";
  ...
  currencyloadfn(){
    ...
    this.CommonService.requestData('BdAMx4fdex/profit',lstinput)
    .subscribe(resData => {
  ...
  //        x.useraddress.trim().toLowerCase().includes(this.defsearch_banner.trim().toLowerCase())
  //     );
  //     this.page_banner.totalElements = this.bannerblocklist1.length;
  //     this.bannerblockrows = this.bannerblocklist1;
  //     this.bannerblocktemp = this.bannerblocklist1;
  ...
  download_csv(){
    var lstinput = {"page":this.page_banner,"sorting":this.defsort_banner,"search": this.defsearch_banner};
    this.CommonService.requestData('BdAMx4fdex/profit',lstinput).subscribe(resData => {
      if(resData){
        var download_data=[]
        for(var i=0;i<resData.Message.length;i++){
          var datas={
            currencySymbol:resData.Message[i].currencySymbol,
            datetime:resData.Message[i].datetime,
            fees:resData.Message[i].fees,
            type:resData.Message[i].type,
            useraddress:resData.Message[i].useraddress
          }
          download_data.push(datas)
        }
        var Userexport = download_data;
        var options={
          headers: ["currencySymbol", "datetime", "fees","type","useraddress"]
        }
        new ngxCsv(Userexport, 'Export Profit details' + new Date().getTime(),options);
      }
    })
  }
  ...
  download_excel():void {
    ...
    this.CommonService.requestData('BdAMx4fdex/profit',lstinput).subscribe(resData => {
      if(resData){
        ...
        this.CommonService.exportAsExcelFile(download_data, 'Export Profit details' + new Date().getTime());
      }
    })    
  }

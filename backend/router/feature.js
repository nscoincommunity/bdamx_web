...
router.get('/getfaqlistcms', (req, res) => {
	try{
		async.parallel({
			bannerData:function(cb) {
			  features.find({status:0}).exec(cb);
	   	},
		},function(err,results){
		console.log(results);
		  if (err) return res.status(500).send(err);
...
router.get('/getnews', (req, res) => {
	try {
			common.redisConfig('features', '', (resData) => {
				var resp=[];
...
router.get('/getnewspage', (req, res) => {
	try{
		features.find({"status":0},{'title':1,"description":1,"modifieddate":1,"content":1}).sort({_id:-1}).exec(function(err,news){
...
router.post('/faqupload',(req, res) => {
	try{
		var values = req.body;
		features.create(values, function (err, resData) {
		   common.SetredisConfig('features', '', (resData) => {
	         if(err) {
...
router.post('/updateimage_nw', (req, res) => {
  features.updateOne({ "_id": req.body._id },{$set:{
		...
		try
		{
			common.SetredisConfig('features', '', (resData) => {
				if(err){
					...
					features.find({ "_id": req.body._id }).exec(function(err,resData){
						if (err) return res.status(500).send(err);
						apiresponse.data = [];
...
router.post('/faqstatus', (req,res) => {
	try{
		features.updateOne({ "_id": req.body._id },{ "$set": { "status": req.body.status,
...
router.post('/deletefaq', (req, res) => {
	...
	features.findOneAndRemove({"_id": req.body._id}).exec(function(err,resData){
		try{
			common.SetredisConfig('features', '', (resData) => {
				if(err){
					res.json({ "status": false});

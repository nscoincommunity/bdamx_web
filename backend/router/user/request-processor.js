...
var abcdMinimumDate = '2019-11-01';
...
// clear cache 
 	RequestProcessor.prototype.clearcache = function () {
		try {
			abcdCache = {};
		} catch (error) {
			console.error('Exception:  request process --------> ' + error);
...
RequestProcessor.prototype._sendSymbolHistory = function (symbol, startDateTimestamp, endDateTimestamp, resolution, response) {
	...
	//console.log(dateForLogs() + "Sending request to abcd  " + key + ". url=" + address);
	 console.log("config.tradeviewHost=======",config.tradeviewHost)
	 console.log("address=======",address)
	httpGet(config.tradeviewHost, address, function (result) {
	// ;pGet("192.168.4.12", address, function (result) {

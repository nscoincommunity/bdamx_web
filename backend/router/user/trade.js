...
var web3 = new Web3("https://infura.io/v3/682d21aed8204bfe8370b50a93aed124");
...
router.post('/getContract', (req,res) => {
	try{
		...
		getJSON(api_hit_url+"api?module=contract&action=getabi&address=0xa94758d328af7ef1815e73053e95b5f86588c16d", function(errorBal,response){
	...
	try{
		if(req.body){
			if(req.body.address)
			{
				...
				/*common.redisConfig('currency', '', (curnRes) => {*/
					/*common.redisConfig('currency', 'status,1', (curnRes) => {*/
					currencyDb.find().exec(function(err,curnRes){
					WalletDB.findOne({user_id : address}).exec(function(err,resdata1){
						...
						console.log(curnRes,'curencyresponse');
			...
			currencyDb.find({ "currencySymbol": second }).select("_id").exec(function (serr, sres) {
			  ...
			  mapDb.aggregate([
				...
				},
				{
			  	...
			  	console.log("result123213==========>>>>>",result)
			  	var final_result	=[]
...
router.post('/get_deposit', (req,res) => {
   try{
	...
	depositDB.find({"userId":req.body.userId}).sort([['createddate', -1]]).exec(function(err, resdata) { res.json(resdata) });
  } catch (e) {
	  res.status(401).send('unauthorized')
  }
})
router.post('/get_withdraw', (req,res) => {
  try{
	...
	WithdrawDB.find({"useraddress":req.body.userId}).sort([['datetime', -1]]).exec(function(err, resdata) { res.json(resdata) });
  } catch (e) {
	  res.status(401).send('unauthorized')
  }
})

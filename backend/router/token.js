...
var abiget = getJSON(api_hit_url+"api?module=contract&action=getabi&address="+getdet.conaddress, function(errorBal,response1){
...
res.json({ "status": true, "Message": "Your Token added successfully. Once it get confirmed in Etherscan token status will be updated.","getdet":TokensRes });
...
res.json({status: false,Message: 'Token not created.please try again'});
    ...
    res.json({status: false,Message: 'Insufficient gas fee price try again',add:address});
});
});
